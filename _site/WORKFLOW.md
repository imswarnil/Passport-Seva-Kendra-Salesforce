...
| **`PSK_Verification_Officer`** | Police-verification officer | `Police_Verification` | Police Verification | Full CRUD on `Police_Verification__c`; create/edit (not delete) on `Risk_Flag__c`; edit (not create/delete) on `Passport_Application__c`; read-only on `Citizen__c`. No access to `Document_Checklist_Item__c` or `Objection__c` — split out to `PSK_Document_Verification_Officer`. **PII fields on the application are fully editable** — a departure from the "must not automatically read the Aadhaar token" principle in PSK.md §5.2; this persona can both read and edit `Aadhaar_Token__c`. All six record types visible, including Diplomatic_Official. | `Application_Management_Console`, `PSK_Police_Verification_Console`, `Passport_Validator` | `Police_Verification__c.Mark_Cleared`, `Police_Verification__c.Mark_Adverse`, `Passport_Application__c.Raise_Risk_Flag` |
...
**The PII field-level-security point from PSK.md §5.2, checked against the real permission sets:** the principle stated there ("a persona that can open a record must not automatically read the Aadhaar token") holds cleanly for `PSK_Reference_Data_Admin` (no access to the object at all), `PSK_Fulfilment_Officer` (read-only, no PII edit), and — since the verification permission sets were split — `PSK_Document_Verification_Officer` (read-only on `Passport_Application__c`, no PII edit). It does **not** hold for `PSK_Verification_Officer` (the police-verification set), `PSK_Granting_Officer`, `PSK_Office_Manager`, or `PSK_Officer` — all four grant full edit access to `Aadhaar_Token__c` and the other PII fields on `Passport_Application__c`. `PSK_Auditor_Read_Only` is a deliberate, documented exception (compliance needs to see the data, just never change it). Whether the remaining four should be tightened to field-level read-only for `Aadhaar_Token__c` specifically is an open question worth raising with whoever owns the security model next.
