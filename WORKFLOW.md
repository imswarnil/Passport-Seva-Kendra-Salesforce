...
Eight permission sets exist in `force-app/main/default/permissionsets/`. `PSK_App_Access` is the admin/verification catch-all (full CRUD + `viewAllRecords` on every PSK object) used for building and testing the org — the seven `PSK_*` sets below are the real job-shaped personas, each granted on top of the base Salesforce user license.
...
|---|---|---|---|---|---|---|
| **`PSK_Officer`** | Front-office / data-entry counter clerk | None (owns records directly, not via queue) | Draft → Submitted → Payment Pending → Paid | Create/edit `Passport_Application__c`, `Citizen__c`, `Appointment__c`, `Document_Checklist_Item__c` (no delete on any). Create-only on `Payment__c` and `Notification_Log__c` (no edit/delete). Read-only on `PSK__c`, `Slot__c`. **PII fields (`Date_of_Birth__c`, `Mobile__c`, `Aadhaar_Verified__c`, `Aadhaar_Token__c`) are fully editable** — this is the role that collects them. Record types visible: Fresh, Re_Issue, Tatkal, Minor, Lost_Damaged — **`Diplomatic_Official` is not in the set**, so this persona genuinely cannot open a diplomatic/official application. | `Application_Management_Console`, `Passport_Validator` | `Citizen__c.New_Passport_Application`, `Citizen__c.New_Family_Member`, `Citizen__c.Verify_KYC`, `Passport_Application__c.Collect_Payment`, `Passport_Application__c.New_Appointment`, `Passport_Application__c.New_Document_Item` |
| **`PSK_Verification_Officer`** | Combined document-verification + police-verification officer | `Document_Verification`, `Police_Verification` | Document Verification, Police Verification | Full CRUD on `Police_Verification__c`, `Objection__c`; edit (not create/delete) on `Document_Checklist_Item__c`; edit (not create/delete) on `Passport_Application__c`; create/edit (not delete) on `Risk_Flag__c`; read-only on `Citizen__c`. **PII fields on the application are fully editable** — a departure from the "must not automatically read the Aadhaar token" principle in PSK.md §5.2; this persona can both read and edit `Aadhaar_Token__c`. All six record types visible, including Diplomatic_Official. | `Application_Management_Console`, `Passport_Validator` | `Document_Checklist_Item__c.Mark_Received`, `Document_Checklist_Item__c.Mark_Verified`, `Police_Verification__c.Mark_Cleared`, `Police_Verification__c.Mark_Adverse`, `Passport_Application__c.New_Objection`, `Passport_Application__c.Raise_Risk_Flag`, `Objection__c.Resolve_Objection` |
| **`PSK_Granting_Officer`** | Grants or rejects applications, issues passports (including Diplomatic/Official) | `Granting` | Granting → Printing | Edit `Passport_Application__c`; create/edit (not delete) `Passport__c`; edit `Objection__c`; read-only `Police_Verification__c` and `Citizen__c`. **PII fields on the application are fully editable here too** — again broader than a strict need-to-know model would suggest. Reads (not edits) `Approval_Status__c`. All six record types visible. | `Application_Management_Console`, `Passport_Validator` | `Passport_Application__c.New_Objection`, `Objection__c.Resolve_Objection` (advance/reject actions come from the sidebar LWC, backed by `PSK_ApplicationActionsController`) |
...
**The PII field-level-security point from PSK.md §5.2, checked against the real permission sets:** the principle stated there ("a persona that can open a record must not automatically read the Aadhaar token") holds cleanly for `PSK_Reference_Data_Admin` (no access to the object at all) and `PSK_Fulfilment_Officer` (read-only, no PII edit). It does **not** hold for `PSK_Verification_Officer`, `PSK_Granting_Officer`, `PSK_Office_Manager`, or `PSK_Officer` — all four grant full edit access to `Aadhaar_Token__c` and the other PII fields on `Passport_Application__c`. `PSK_Auditor_Read_Only` is a deliberate, documented exception (compliance needs to see the data, just never change it). Whether the other four should be tightened to field-level read-only for `Aadhaar_Token__c` specifically is an open question worth raising with whoever owns the security model next.
...
- **Approver:** a single step, "Regional Passport Officer Sign-off," hard-assigned to the user `admin@passportoffice.com`. Per the session notes, this is a **placeholder** — role-based approver routing to the `Regional_Passport_Officer` role hit metadata-format issues this session and was worked around by pointing directly at the admin user. A real deployment should route this to the role once that's resolved.
...
- **Verifying documents** — a `Document_Checklist_Item__c` being auto-*seeded* does not mean it's auto-*verified*; a `PSK_Verification_Officer` has to actually inspect each one and click `Mark_Received` / `Mark_Verified`.
...
- **Deciding the Diplomatic/Official grant approval** — approve or reject is a human call by the assigned Regional Passport Officer (currently the admin user placeholder); the system only handles the plumbing of submission, field updates, and routing the record back a stage on rejection.
